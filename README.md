...
- `local_approval_server.py`：同源本机桥接服务，只接受月份和精确选择标识，后台执行并返回结构化结果。
...
- `manage_weekly_refresh.ps1`：管理每周一 09:00 的当前用户刷新任务；它不会自动审批。
...
直接打开 `工时审批看板_多月.html` 即可查看和选择。点击页面中的“直接审批已选明细”会自动连接后台服务并转交当前选择；不需要双击任何启动脚本。如果页面已打开，重新生成后应强制刷新以加载新版本。

当前用户已安装每周一 09:00 的刷新任务：它只读取 SRDPM 当前月数据、重跑审计并更新页面，绝不会调用审批或驳回接口。刷新失败时旧归档和旧页面保持不变；首次运行前需先在 UI 中配置一次 Windows Credential Manager 凭据。
