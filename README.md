...
- `wiki_project_mapping.py`：只读检查固定 Wiki 页面、严格校验最新附件并生成当前项目/机芯授权映射。
- `project_mapping.json`：`person_projects` 保存最新 Wiki 当前规则，`authorization_retention` 保存人员从机芯撤出后的历史宽限周期。
- `chip_history.json`：只增不减的历史机芯注册表；只用于识别曾出现过的机芯，不能单独恢复人员授权。
...
- `approval_readback_store.py`：按月原子保存服务器逐 ID 回读通过的证据，并以身份约束覆盖本地抓取快照的显示状态。
...
- `refresh_dashboard.py`：同步 Wiki 映射，只读抓取当前月和前一月、审计并把映射/归档/历史机芯/看板作为一批发布；不包含审批功能。
...
页面直批的凭据由 UI 首次配置到当前 Windows 用户的 Credential Manager，不会写入项目文件。Wiki 只读附件访问使用当前用户环境变量 `IDISPLAYVISION_WIKI_PAT`；只保存变量名，不得把真实 PAT 写进项目、命令参数或日志。命令行备用执行器仍可通过当前进程环境变量提供 SRDPM 凭据：
...
直接打开 `工时审批看板_多月.html` 即可查看和选择。点击页面中的“直接审批已选明细”会自动连接后台服务并转交当前选择；不需要双击任何启动脚本。顶部“↻ 同步 Wiki 并读取近两月”会先检查页面 22824730 的最新日期版附件，只读取与附件年月一致的 `计算负荷用-YYYY-M月` 工作表，展开合并单元格后更新允许机芯；随后读取 SRDPM 当前自然月和前一个自然月、重跑审计并自动加载最新看板。它不会审批，且成功后会清除这两个月旧的本地选择，避免把旧快照的勾选带入新数据。如果页面已打开，重新生成后应强制刷新以加载新版本。

当前用户已安装每周一 10:00 的刷新任务：它先只读检查 Wiki 最新项目负荷附件，再读取 SRDPM 当前月和前一个月数据、重跑审计并更新页面，绝不会调用审批或驳回接口。页面手动刷新与该定时任务、真实审批相互排斥；Wiki 凭据缺失、附件下载不完整、文件不是 XLSX、大小/表头/月份/合并区域异常，或 SRDPM 任一步失败时，旧映射、旧归档、历史机芯和旧页面全部保持不变。首次运行前需先在 UI 中配置一次 Windows Credential Manager 凭据，并确保当前用户环境已配置 `IDISPLAYVISION_WIKI_PAT`。若从 `file://` 直接打开旧静态页，刷新按钮会先安全地打开本机看板；请在该同源页面再点击一次，避免外部页面诱发带凭据刷新。
...
机芯规则分为三层：①“当前规则”来自最新 Wiki 的 `person_projects`；②人员从某机芯撤出后，撤出当月及随后两个完整自然月属于“历史合理规则”，仍判为正常并在审核原因中标明有效截止月份；③超过截止月份后转为“历史过期规则”，继续保留用于解释，但不再放行。例如 2026 年 7 月首次确认撤出，2026 年 7、8、9 月的工时仍合理，2026 年 10 月起才判为过期。判断依据是工时记录所属月份，不是重新加载页面的日期；同一附件反复刷新不会延长期限，恢复后再次撤出会新建一个带重新加入月份下界的周期，不会把中间空档月倒推为合理。历史宽限只匹配人员与明确机芯，不会扩大成旧客户下的所有项目。

每次审计还会把这三层规则按工时月固化到该月归档。看板“三、工时异常”和“四、项目归属异常”分别展示“当前规则机芯、历史合理机芯、历史过期机芯”三列，合理与过期项会显示各自截止月份；第四类的三列替代旧的“允许机芯”合并列。回看旧月份时只读取当月快照，不会因之后的 Wiki 更新而漂移；最新 Wiki 附件没有该人员时显示“Wiki 未列此人”，旧归档若尚未保存快照则显示“旧归档未记录（需重新审计该月）”。

机芯识别和人员授权仍相互分离：每次审计会把工作表出现过且可识别的新机芯合并进 `chip_history.json`，历史库只增不减，但它本身不会为任何人员授权。项目实值已验证 `T` 与 `AM` 前缀在数字主体相同时属于同一机芯家族（如 `T950`↔`AM950`、`T963`↔`AM963`、`T966`↔`AM966`）；项目名明确写出的 `D4` 与 `D5` 版本仍严格区分并优先于前面的模糊整机编码。机芯别名自身含 `/`（如 `MT9603/L`）时不会再被误当成“客户/机芯”分隔符，紧跟中文描述的“MT9603L机芯”也会被提取。授权宽限或机芯历史缺失、损坏时刷新直接停止，避免静默遗忘。
...
- 服务器未回读成功时绝不显示“已审批”。
- 服务器已回读并保存后，整页刷新和历史月份重开仍显示“已审批”；部分成功只保存已确认通过的 ID。
