...
Wiki 页面 22824730 的最新项目负荷附件
        |
        v
wiki_project_mapping.py --> project_mapping.json（当前允许范围）
        |
        v
fetch_and_audit.py
        |
...
- `project_mapping.json`：由最新 Wiki 当月工作表生成的项目/机芯与人员当前规则映射。
- `chip_history.json`：只增不减的历史机芯注册表；只用于识别曾出现过的机芯，不保留或恢复旧的人员授权。
...
机芯识别采用“当前授权”和“历史记忆”分离：每次手动或周一刷新都会检查 Wiki 最新附件，`project_mapping.json` 只代表最新当月工作表中的当前允许范围；每次审计会把工作表出现过且可识别的新机芯合并进 `chip_history.json`，历史库只增不减。Wiki 删除旧机芯后，旧机芯仍能被识别，但不会继续获得原人员授权，会显示为历史机芯且按当前范围判为不匹配。历史库缺失或损坏时刷新直接停止，避免静默遗忘。项目实值已验证 `T` 与 `AM` 前缀在数字主体相同时属于同一机芯家族（如 `T950`↔`AM950`、`T963`↔`AM963`、`T966`↔`AM966`）；项目名明确写出的 `D4` 与 `D5` 版本仍严格区分并优先于前面的模糊整机编码。机芯别名自身含 `/`（如 `MT9603/L`）时不会再被误当成“客户/机芯”分隔符，紧跟中文描述的“MT9603L机芯”也会被提取。
