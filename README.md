...
直接双击 HTML 时仍是完全静态的离线看板，不会执行真实审批。需要一键审批时，双击 `启动工时审批看板.cmd`：它只在 `127.0.0.1` 启动同源本机服务并打开页面。页面只提交月份和稳定整单标识；服务端从当前归档重建人员、日期和审批 ID，并在每个整单提交前复检、提交后逐 ID 回读。只有回读明确为“通过”的整单才会显示已审批。本项目的自动化测试全部使用假客户端，不会连接或修改 SRDPM。
...
- `启动工时审批看板.cmd`：推荐入口；启动时安全读取凭据并打开本机看板。
...
如果没有预先设置环境变量，推荐入口会在启动时询问用户名和密码；密码输入不会回显，只保留在该服务进程内存中，关闭服务窗口后即清除。
...
离线查看可直接打开 `工时审批看板_多月.html`。需要页面内直接审批时，请双击 `启动工时审批看板.cmd`，不要直接打开 HTML。如果页面已打开，重新生成后应强制刷新以加载新版本。
