...
机芯识别采用“当前授权”和“历史记忆”分离：`project_mapping.json` 只代表当前 Wiki 允许范围；每次审计会把其中的新机芯合并进 `chip_history.json`，历史库只增不减。Wiki 删除旧机芯后，旧机芯仍能被识别，但不会继续获得原人员授权，会显示为历史机芯且按当前范围判为不匹配。历史库缺失或损坏时刷新直接停止，避免静默遗忘。明确写出的 `D4` 与 `D5` 版本严格区分。
